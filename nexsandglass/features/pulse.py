...
def pulse(user_message: str = "") -> str:
    ...
    try:
        from nexsandglass.core.emotion_vocab import detect as emotion_detect, mood_message, learn as emotion_learn
    ...
    # 决策粒子自动触发——三步过滤防闲话
    try:
        from nexsandglass.features.decision_particles import log as dp_log, _detect_chain, _is_decision
    ...
    # ── 偏移率觉察（玻璃模式）──
    try:
        from nexsandglass.features.sandglass_think import glass_reminder, persona_freshness
    ...
    # 对比今昔——搜过去相关的话题
    from nexsandglass.features.sandglass_vault import count as sv_count, search
    total = sv_count()
    ...
    try:
        ...
        if total % 100 == 0 and total > 0 and not os.path.exists(_MILESTONE_FLAG):
            ...
            if total == 100:
                # 首次画像生成
                try:
                    from nexsandglass.features.sandglass_think import persona_build
                    persona_build()
                ...
                # 自动画像维护
                try:
                    from nexsandglass.features.sandglass_think import persona_maintain
                    r = persona_maintain()
                    if r.get("triggered"):
                ...
                # 阶段切换启用 + 冷迁移
                try:
                    from nexsandglass.features.sandglass_think import _maybe_switch_stage
                    _maybe_switch_stage("neutral")
                    signals.insert(0, "📜 沙漏满1000条——阶段切换系统启用")
                except Exception as e:
                    logger.warning(f"里程碑1000: {e}")
                try:
                    from nexsandglass.core.sandglass_archive import cold_migration
                ...
                # 3D已解锁，每次+500触发重合成
                try:
                    from nexsandglass.features.sandglass_think import _should_synthesize, _synthesize_3d
