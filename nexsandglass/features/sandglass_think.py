...
from nexsandglass.l3.scene_l3 import (
...
from nexsandglass.l3.weave_l3 import (
    weave_insight, weave_contradiction, weave_chain, weave_graph,
)
from nexsandglass.l3.emotion_l3 import (
    entropy_mirror, entropy_ghost, glass_reminder,
    entropy_reminder, memo_mode,
)
from nexsandglass.utils.discipline import iron_rules, iron_rules_set
from nexsandglass.l3.l3_tasks import task_defer, task_pending, task_done, task_check_trigger
from nexsandglass.l3.l3_persona_verify import persona_trace, persona_verify, persona_diff
from nexsandglass.l3.l3_search_core import _synonym_expand, _tfidf_search, composite_rerank, _search_with_fallback, _sentiment_wind, sentiment_rerank, simhash_search
from nexsandglass.l3.l3_persona import persona_project
from nexsandglass.l3.persona_l3 import (
...
from nexsandglass.l3.persona_l3 import _WAVE_THRESHOLDS, _SEARCH_WEIGHTS
from nexsandglass.l3.offset_l3 import (
...
from nexsandglass.l3.offset_signals import _fail_open
...
def full_sanity() -> dict:
    ...
    # L1
    try:
        from nexsandglass.features.nightwatch import night_watch
        nw = night_watch()
    ...
    # L2
    try:
        from nexsandglass.features.sandglass_vault import count, search
        total = count()
    ...
    try:
        ...
        checks = {}
        try:
            from nexsandglass.features.sandglass_think import _emotional_entropy
        ...
        try:
            from nexsandglass.features.sandglass_think import weave_contradiction
            w = weave_contradiction()
            checks["织布机"] = f"✅ {len(w.get('conflicts',[]))}处矛盾"
        except: checks["织布机"] = "❌"
        try:
            from nexsandglass.features.sandglass_think import stage_list
            s = stage_list()
            checks["阶段"] = f"✅ {len(s)}阶段"
        except: checks["阶段"] = "❌"
        try:
            from nexsandglass.features.sandglass_think import search_filter
            sf = search_filter("test")
            checks["搜索"] = "✅" if sf.get("keywords") else "⚠️"
        except: checks["搜索"] = "❌"
        try:
            from nexsandglass.features.decision_particles import _detect_chain
            c = _detect_chain("选A还是B")
            checks["决策粒子"] = "✅" if isinstance(c, list) else "⚠️"
        except: checks["决策粒子"] = "❌"
        try:
            from nexsandglass.features.sandglass_think import scene_stage_matrix
            ssm = scene_stage_matrix()
            checks["场景矩阵"] = f"✅ {len(ssm.get('stages',[]))}阶段×{len(ssm.get('scenes',[]))}场景"
        except: checks["场景矩阵"] = "❌"
        try:
            from nexsandglass.features.sandglass_think import scene_stage_cross_validate
        ...
        try:
            from nexsandglass.features.sandglass_think import entropy_mirror
            em = entropy_mirror("最近决策")
            checks["熵镜"] = "✅" if em.get("found_mirror") or "无匹配" in str(em) else "✅"
        except: checks["熵镜"] = "❌"
        try:
            from nexsandglass.features.sandglass_think import entropy_ghost
...
def persona_maintain() -> dict:
    ...
    # 24h 最小间隔检查
    from nexsandglass.l3.persona_l3 import _PERSONA as _PERSONA_FILE
