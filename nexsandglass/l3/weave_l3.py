...
try:
    from nexsandglass.features.sandglass_think import _fail_open, _llm, comprehensive_offset, cross_stage_offset, stage_list, search_with_stage_label, weave_links
except:
...
def weave_insight(topic: str) -> dict:
    ...
    result["thread_view"] = ""
    try:
        from nexsandglass.features.weavethread import wthread_stats, wthread_to_weave
        stats = wthread_stats()
...
def weave_contradiction() -> dict:
    ...
    # 矛盾2：场景占比变了但阶段没切
    try:
        from nexsandglass.l3.scene_l3 import scene_dominance
    except:
        from nexsandglass.features.sandglass_think import scene_dominance
    dom = scene_dominance()
    if dom.get("shift"):
        for s in dom["shift"]:
            if abs(s["delta"]) >= 30:
    ...
    # 矛盾3：稳定性低但无切换预测
    try:
        from nexsandglass.features.sandglass_think import decision_stability
    except:
        decision_stability = lambda: {"overall": {"volatility": 0}}
    stab = decision_stability()
    try:
        from nexsandglass.l3.scene_l3 import stage_switch_prediction
    except:
    ...
    try:
        from nexsandglass.features.sandglass_think import _latest_annotation
    except:
