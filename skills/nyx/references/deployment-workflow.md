...
### 1. Upload file
```bash
scp /root/nyx_viz.html root@REDACTED_IP:/var/www/nyx/index.html
```
...
### 4. SSL Certificate
```bash
certbot --nginx -d nyx.hvh.expert --non-interactive --agree-tos -m REDACTED_EMAIL --redirect
```
...
**Solution**: Full key is in `REDACTED_CREDENTIALS_PATH`. Extract with:
```bash
（凭据按需从安全存储读取）
```
...
curl -s -X POST "https://api.cloudflare.com/client/v4/zones/$ZONE_ID/dns_records" \
  ...
  --data '{"type":"A","name":"nyx","content":"REDACTED_IP","proxied":false,"ttl":1}'
```
